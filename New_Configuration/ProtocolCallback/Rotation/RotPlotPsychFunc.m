function RotPlotPsychFunc(appHandle)

global debug

if debug
    disp('Entering RotPlotPsychFunc')
end
...
str(2) = {['Trial Completed: ' num2str(trial.cntr)]};
str(3) = {['Trial Remaining: ' num2str(trial.num-trial.cntr)]};   
str(4) = {['Total Correct: ' num2str(sum(cell2mat({Resp.totalCorr})))]};
str(5) = {['Total Incorrect: ' num2str(sum(cell2mat({Resp.totalIncorr})))]};
str(6) = {['Total Null Trials: ' num2str(sum(cell2mat({Resp.totalNull})))]};
str(7) = {['Total Don''t Know Trials: ' num2str(sum(cell2mat({Resp.totalDontKnow})))]};
set(findobj(appHandle,'Tag','TrialInfoText'),'String',str);


if debug
    disp('Exiting RotPlotPsychFunc')
end
