...
if data.configinfo(i).parameters == 1   % For single interval
    ...
    % New code to load and save depth offset instead of direction
    if ~isempty(strmatch('Depth Offset',{char(within.name)},'exact'))
        %i1 = strmatch('Heading Direction',{char(data.condvect.name)},'exact');
        %dir(1) = crossVals(trial.list(trial.cntr),i1);
        DO = within.parameters((trial(activeStair,activeRule).list(currTrial)));
    else
        i = strmatch('DEPTH_OFFSET',{char(data.configinfo.name)},'exact');
        DO = data.configinfo(i).parameters;
    end

%     savedInfo(activeStair,activeRule).Resp(currRep).dir(currTrial,:) = dir2 - dir1;
    savedInfo(activeStair,activeRule).Resp(currRep).dir(currTrial,:) = DO;
